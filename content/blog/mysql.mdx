...
-- os campos e o que vai receber de alteração, o where é a verificação de onde alterar e o
--limit é uma segurança para alterar somente 1 registro

DELETE FROM nomedatabela
WHERE chaveprimaria = '1'
LIMIT 2;--Deleta o(s) registro(s) passado(s) no where

TRUNCATE TABLE nomedatabela;--Deleta TODOS os registros da tabela, mas não deleta a 
--estrutura da tabela
```
...
select * from nomedatabela order by nome; --Order by ordena pelo parâmetro passado
--nesse caso, ordena pelo nome em ordem ascendente A-Z, 1-2 

select * from nomedatabela order by nome desc; --Order by ordena pelo parâmetro passado
--nesse caso, ordena pelo nome em ordem descentente Z-A, 2-1

select * from nomedatabela where id = '13'; --Seleciona tudo que atenda a condição no where
...
--pareça com o parêmetro passado em parênteses
select * from nomedatabela where nome not like 'silva';--O not lke é o contrário do like
...
select * from nomedatabela where nome like '%silva';--Irá pegar tudo que termine com silva
select * from nomedatabela where nome like '%silva';--Irá pegar tudo que começe com silva
select * from nomedatabela where nome like '%silva%';--Irá pegar tudo que tenha silva em
--qualquer lugar

--O "_" se assemelhar ao "%", porém a diferença é que obrigatoriamente precisa existir um
--caratere para puxar, o % é opcional a existência desse caractere
...
select avg(totalaulas) from cursos;--Retorna a média dos valores

select totalaulas from cursos group by totalaulas;--Funciona de forma parecida do
--distinct porém dessa forma conseguimos contar cada grupo

select totalaulas from cursos group by ano having ano > 2016;--Funciona de forma parecida
--do where no select, porém só pode ser passado o mesmo parâmetro do group by
```
...
Em SQL, os atributos são as colunas de uma tabela. Cada atributo corresponde a um tipo de dado específico, como texto, número, data, entre outros. Os atributos definem a estrutura da tabela e determinam quais tipos de valores podem ser armazenados em cada coluna.
...
Para cadastrar uma chave estrangeira em uma tabela, primeiro precisamos identificar a relação como diz acima, `1 pra 1`, `1 pra N` ou então `N pra N` , feito isso seguimos as seguintes regras:

- **1 pra 1**
No relacionamento `1 pra 1`, colocamos a chave estrangeira na entidade “dominante”, como no exemplo abaixo:
...
- **N pra N**
    ...
    O relacionamento `N pra N` é um pouco mais complexo, precisamos criar outra entidade entre as duas que estamos relacionando e liga-las com relacionamentos `1 pra n`, seguindo a regra de `1 pra N` , como no exemplo abaixo:
...
- **Inner Join**
    ...
    //Basicamente retorna os dados que tiverem relação, ou seja, só irá mostrar os
...
- **Outer Join**
    ...
    on c.idcurso = g.cursopreferido;
    //Retorna todos os dados da tabela direcionada(left, righ, full) + as relações da
    //outra tabela.
    ```
    

<aside>
💡 Tanto no inner join como no outer join, as palavras inner e outer são opcionais, porém no outer join, precisa-se referenciar qual outer join é, left, right ou full.
...
Primeiro criamos uma tabela ligando as chaves primárias de ambas as tabelas:
    ...
    Depois, utilizamos um join seguido do outro para ligar os dados de ambas as três tabelas: